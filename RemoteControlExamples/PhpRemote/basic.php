...
echo "<center><h1> - Trackmania dedicated server - </h1></center>";
...
// do the job.
// ----------------------------------------------------------------
$SimpleActions = array('ChallengeRestart', 'NextChallenge', 'StopServer', 'QuitGame', 'GenerateCoreDump');
...
} else if ($Action == 'Kick' || $Action == 'Ban' || $Action == 'AddGuest' || $Action == 'RemoveGuest' || $Action == 'BlackList' || $Action == 'UnBlackList') {
...
END;
		...
		else
		{
			...
			echo "Current challenge : " . $CurrentChallengeInfo['UId'] . " - " . $CurrentChallengeInfo['Name'] . "<BR>";
		}
		...
		if( $Status['Code'] == 4 )
		{
			echo <<<END
			<form name="input" action="basic.php" method="post">
			<input type="submit" name="action" value="ChallengeRestart"/>
...
echo $repAllowChallengeDownload."\n";
	...
	if( $repNextLadderMode==0 )
	{
		...
		$NextLadderModeNormal = "";
		$NextLadderModeForced = "";
	}
	else if( $repNextLadderMode==2 )
	{
	...
	<SELECT name="NextLadderMode" >
		...
		<OPTION value="1" $NextLadderModeNormal> Normal </OPTION>
		<OPTION value="2" $NextLadderModeForced> Forced </OPTION>
	</SELECT>
	</td></table></td></tr>
...
// ----------------------------------------------------------------
// debug
// Challenges\TM-Community\Sixtyniner.Challenge.Gbx
// Challenges\TM-Community\CoastALOs.Challenge.Gbx
echo "\n<h3>Challenges:</h3>\n";
...
else
{
	...
	foreach ($ChallengeList as $challenge) {
		...
		echo <<<END
		<TR><TD>$ChallengeUId</TD><TD>$FileName</TD><TD>$Environnement</TD><TD>$GoldTime</TD><TD>$CopperPrice</TD>
